...
if [ "$1" = "" ]
then
    echo "You forgot to add the assignment name, e.g. 'Formative1'."
    echo "Please run the script again with the right argument."
    exit 1
fi

#if hostname -f | grep "project-" > /dev/null; then
if ghc --version | grep 'version 8.6.5' > /dev/null; then
    ...
    echo "Please ensure you are on CoCalc with the right settings."
    exit 1
fi
#else
#    echo "Please run this script on CoCalc."
#    exit 1
#fi
...
then
    ...
    echo -e "\t(i) your file is called something like 'FormativeX.hs'"
